...
args="-all ${topo_path}/{2}.dot ${demand_path}/{2}-matrix/{2}_mergelen_12 ${demand_path}/{2}-matrix/{2}_mergelen_12_corr_noise_{3}  ${host_path}/{2}.hosts -num-tms ${num_tm} -deloop -scalesyn -budget ${budget} -scale {1} -out ${out_dir}"
