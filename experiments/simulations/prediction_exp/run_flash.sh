...
num_tm=4
budget=3
flash="-flash-tm 3 -flash-det-delay 1 -flash-ba {3}"
flash_name=$(echo "${flash}" | tr '[[:space:]]' '_')
out_dir="flash_{2}_scale_{1}_budget_${budget}_numtm_${num_tm}_${flash_name}"

executable="${kulfi_path}/Simulate_Driver.native"
args="-all ${topo_path}/{2}.dot ${demand_path}/{2}-matrix/{2}_mergelen_12 ${demand_path}/{2}-matrix/{2}_mergelen_12  ${host_path}/{2}.hosts ${num_tm} -deloop -scalesyn -budget ${budget} -scale {1} ${flash} -out ${out_dir}"
