...
echo "Same as budget experiment!!"
exit 0
...
num_tm=24
budget=3
out_dir="v2_scale_{2}_scale_{1}_budget_${budget}_numtm_${num_tm}"

executable="${kulfi_path}/Simulate_Driver.native"
args="-all ${topo_path}/{2}.dot ${demand_path}/{2}-matrix/{2}_mergelen_12 ${demand_path}/{2}-matrix/{2}_mergelen_12  ${host_path}/{2}.hosts -num-tms ${num_tm} -deloop -scalesyn -budget ${budget} -scale {1} -out ${out_dir}"
