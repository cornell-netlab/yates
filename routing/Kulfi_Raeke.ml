...
let solve (t:topology) (_:demands) (_:scheme) : scheme =
  let epsilon = 0.1 in 
  ...
  let paths src dst : PathProbabilitySet.t = 
    List.fold_left mw_solution 
      ...
      ~f:(fun acc (rt,p) -> 
        ...
        let x : PathProbabilitySet.t = PathProbabilitySet.add (physical_path,p) acc in 
        x) in   
  Topology.VertexSet.fold 
    ...
    ~f:(fun acc src -> 
      Topology.VertexSet.fold 
        ...
        ~f:(fun acc dst -> 
          if src <> dst then 
            SrcDstMap.add (src, dst) (paths src dst) acc  
          else 
            acc))
