...
let minus ex1 ex2 =
  ...
  let rec negate ex = match ex with
    ...
    | Num f -> [Num (-.f)]
    | Times (f, x) -> [Times (-.f, x)]
    | Sum lst -> List.flatten (List.map (fun x -> negate x) lst) in
...
let capacity_constraints (topo : Topology.t) (d_pairs : demand_pair list)
  ...
  Topology.fold_edges (fun edge acc ->
      ...
      let all_flows = List.fold_left (fun acc2 pair ->
          let forward_amt = var_name topo edge pair in
	  (*
            let reverse_amt = var_name_rev topo edge pair in
	  *)
          Var(forward_amt):: (*Var(reverse_amt)::*) acc2) [] d_pairs in
      (* Add them all up *)
...
let demand_constraints (topo : Topology.t) (d_pairs : demand_pair list)
  ...
  List.fold_left (fun acc d_pair ->
      ...
      let diffs = List.fold_left (fun acc2 edge ->
      ...
      (Geq (name, sum_net_outgoing,
            demand /. demand_divisor))::acc) init_acc d_pairs
  ...
  List.fold_left (fun acc d_pair ->
      ...
      Topology.fold_vertexes (fun v acc2 ->
          if v = src || v = dst then acc2 else
            ...
            let outgoing = List.fold_left (fun acc_vars e ->
                (Var (var_name topo e d_pair))::acc_vars) [] edges in
            let incoming = List.fold_left (fun acc_vars e ->
                (Var (var_name_rev topo e d_pair))::acc_vars) [] edges in
            ...
            constr::acc2) topo acc) init_acc d_pairs
  ...
  | Sum (aexs) ->
    List.fold_left (fun acc ae ->
        if acc = "" then string_of_aexp ae else match ae with
          ...
          | _ -> acc ^ " + " ^ (string_of_aexp ae)) "" aexs

let string_of_constraint c =
  match c with
  | Eq (name, ae, f) ->
    Printf.sprintf "%s: %s = %s" name (string_of_aexp ae) (string_of_float f)
  | Leq (name, ae, f) ->
    Printf.sprintf "%s: %s <= %s" name (string_of_aexp ae) (string_of_float f)
  | Geq (name, ae, f) ->
    Printf.sprintf "%s: %s >= %s" name (string_of_aexp ae) (string_of_float f)
...
let serialize_lp ((objective, constrs) : lp) (filename : string) =
  let lp_file = open_out filename in
  ...
  List.iter (fun c -> output_string lp_file
                (Printf.sprintf "  %s\n" (string_of_constraint c))) constrs;
  close_out lp_file
...
let names_of_path topo path =
  let path_dsts = List.map (fun e -> fst (Topology.edge_dst e)) path in
  let path_verts = (fst (Topology.edge_src (List.hd path)))::path_dsts in
  List.map (name_of_vertex topo) path_verts
...
let recover_paths (orig_topo : Topology.t) (flow_table : flow_table) =
  ...
  let strip_paths (d_src, d_dst) edges =
    ...
    (* Add all the nodes *)
    let node_table = Hashtbl.create ~random:false 8 in
    (* Adds a node if it has not already been added *)
    let add topo node =
      if Hashtbl.mem node_table node then topo else
        let (new_topo, node_id) = Topology.add_vertex topo node in
        let () = Hashtbl.add node_table node node_id in
        new_topo in
    ...
    let topo_with_nodes = List.fold_left (fun acc_topo (src,dst,_) ->
        let acc_topo2 = add acc_topo src in
        add acc_topo2 dst) topo_with_src_dst edges in
    (* Add all the edges *)
    let id_of_node n = Hashtbl.find node_table n in
    let topo_with_edges = List.fold_left (fun acc_topo (src,dst,cap) ->
        ...
        new_topo) topo_with_nodes edges in
    ...
    let rec find_paths path_topo acc_paths =
      ...
      | Some path ->
        (* Find bottleneck *)
        let bottleneck = List.fold_left (fun acc edge ->
            let label = Topology.edge_to_label path_topo edge in
            min acc (Link.weight label)) max_float path in
        (* Decrease weights by bottleneck, mark those with zero weight
           for deletion *)
        let delete_these = List.fold_left (fun acc edge ->
            let label = Topology.edge_to_label path_topo edge in
            let old_wt = Link.weight label in
            let new_wt = old_wt -. bottleneck in
            Link.set_weight label new_wt;
            if new_wt <= 0. then edge::acc else acc) [] path in
        (* Delete the edges that were zeroed out *)
        let new_topo = List.fold_left (fun acc_topo edge ->
            Topology.remove_edge acc_topo edge) path_topo delete_these in

        let mapped_path = List.map (fun edge ->
  ...
  Hashtbl.fold (fun d_pair edges acc ->
      let (s,t) = d_pair in
      let paths = strip_paths d_pair edges in
      (s,t, paths)::acc) flow_table []
...
let solver_paths topo pairs verbose =
    ...
    let lp_filename = Printf.sprintf "mcf-%d.lp" !iter in
    ...
    let gurobi_in = Unix.open_process_in
        ("gurobi_cl OptimalityTol=1e-9 ResultFile=mcf.sol " ^ lp_filename) in
    ...
    let rec read_output gurobi solve_time =
      try
        ...
        if Str.string_match time_regex line 0 then
          let num_seconds = float_of_string (Str.matched_group 1 line) in
    ...
    let read_results input =
          ...
          let new_z, new_flows =
            if line.[0] = '#' then (opt_z, flows)
              ...
              (if Str.string_match regex line 0 then
                 ...
                 let flow_amt = float_of_string (Str.matched_group 5 line) in
      ...
      close_in results; result in

    let ratio, flows = read_results "mcf.sol" in

    let flows_table = Hashtbl.create ~random:false 8 in

    (* partition the edge flows based on which commodity they are *)
    List.iter (fun (d_src, d_dst, flow, e_src, e_dst) ->
        if Hashtbl.mem flows_table (d_src, d_dst) then
          let prev_edges = Hashtbl.find flows_table (d_src, d_dst) in
          Hashtbl.replace flows_table (d_src, d_dst)
            ((e_src, e_dst, flow)::prev_edges)
        else
          Hashtbl.add flows_table (d_src, d_dst) [(e_src, e_dst, flow)])
      flows;
    ...
    if verbose then
      (Printf.printf "Paths stripped in %f seconds\n" paths_time;
       flush stdout);
    let num_paths = List.fold_left (fun acc (_,_,paths) ->
        acc + (List.length paths)) 0 paths in
