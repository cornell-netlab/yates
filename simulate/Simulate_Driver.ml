            ...
            ~f:(fun ~key:path ~data:prob acc ->
                List.fold_left path
                  ...
                  ~f:(fun acc e ->
                      ...
                      | Some x ->  EdgeMap.add ~key:e ~data:((demand *. prob) +. x) acc))) in
  EdgeMap.fold
    ~init:EdgeMap.empty
...
let curr_capacity_of_edge (topo:topology) (link:edge) (fail:failure) : float =
  if EdgeSet.mem fail link then 0.0
...
let rec get_max_util_failure (topo:topology) (num_fail:int) (utils: congestion EdgeMap.t): failure =
  ...
  let edge_utils = List.filter
      (EdgeMap.to_alist utils)
      ~f:(fun (e,_) -> edge_connects_switches e topo) in
  let sorted_edge_utils = List.sort
    ~cmp:(fun x y -> Float.compare (snd y) (snd x))
    edge_utils in
  let max_util_links,_ =
    list_first_n sorted_edge_utils num_fail
    |> split_alist in
  let fail_links =
    List.fold_left max_util_links
      ~init:max_util_links
      ~f:(fun acc e -> (reverse_edge e)::acc)
    |> EdgeSet.of_list in
  if check_connectivity_after_failure topo fail_links then fail_links
  else
    let tail_utils =
      List.hd_exn sorted_edge_utils
      |> fst
      |> EdgeMap.remove utils in
    get_max_util_failure topo num_fail tail_utils
...
(*  SrcDstMap.iter ~f:(fun ~key:(s,d) ~data:path ->
  ...
  let exp_utils = (congestion_of_paths topo actual spf_scheme) in
...
let pick_flash_sinks (topo:topology) (iters:int) =
  ...
  ~f:(fun acc n ->
    ...
    (*let selected = match List.nth hosts 4 with (*NOTE TODO: selecting h5. change back ^^^^^^*)*)
      | None -> assert false
      | Some x -> x in
    selected::acc)
...
let simulate_tm (start_scheme:scheme) (topo:topology) (dem:demands) (fail_edges:failure) (predict:demands) (algorithm:solver_type) (is_flash:bool) (flash_ba:float) (flash_sink) =
  ...
  * *)
            ...
            if flash_t % flash_step_time = 0 then
              begin (* update actual & pred demand  and scheme *)
                ...
                let sch' = match algorithm with
                  ...
                  | _ -> if !Kulfi_Globals.flash_recover then (select_local_recovery algorithm) current_state.scheme topo failed_links pred'
  ...
  (* Done generating traffic at source *)
      ...
      let next_iter_traffic, new_delivered_map, new_lat_tput_map_map, new_link_utils, new_fail_drop, new_cong_drop =
        ...
        ~f:(fun ~key:e ~data:in_queue_edge link_iter_acc ->
          ...
          let new_lutil_map = if iter < num_iterations + steady_state_time then
            begin
  ...
  (* Generate stats *)
  let tput = SrcDstMap.fold final_network_state.delivered
    ~init:SrcDstMap.empty
    ~f:(fun ~key:sd ~data:dlvd acc ->
      SrcDstMap.add ~key:sd ~data:(dlvd /. (Float.of_int num_iterations) /. !agg_dem) acc) in

  let latency_dist = (get_aggregate_latency final_network_state.latency num_iterations) in

  let congestions = EdgeMap.fold final_network_state.utilization
    ~init:EdgeMap.empty
    ~f:(fun ~key:e ~data:util_list acc ->
      EdgeMap.add ~key:e
        ...
        acc) in
  let fail_drop = final_network_state.failure_drop /. (Float.of_int num_iterations) /. !agg_dem in
  let cong_drop = final_network_state.congestion_drop /. (Float.of_int num_iterations) /. !agg_dem in
  let sink_tput = if is_flash then SrcDstMap.fold final_network_state.delivered
    ~init:0.0
  ...
  tput, latency_dist, congestions, fail_drop, cong_drop, !agg_dem, !recovery_churn, final_network_state.scheme, !solver_time, sink_tput
  (* end simulate_tm *)
...
let get_num_paths (s:scheme) : float =
  let count = SrcDstMap.fold ~init:0
    ~f:(fun ~key:_ ~data:d acc ->
      acc + (PathMap.length d)) s in
  Float.of_int count
...
let simulate
  ...
  let failure_scenarios = get_failure_scenarios topo demand_file host_file iterations num_failures scale in
        ...
        List.fold_left
          ...
          ~f:(fun scheme n ->
            ...
            let list_of_avg_congestions,list_of_max_congestions =
              List.map ~f:snd (EdgeMap.to_alist congestions)
              |> split_alist in
            let max_congestions_map = EdgeMap.map ~f:snd congestions in
            ...
            add_record edge_congestion_data sname {iteration = n; edge_congestions=max_congestions_map; };
...
let calculate_syn_scale (topology:string) (demand_file:string) (host_file:string) =
  ...
  let expected_congestions = congestion_of_paths topo actual s in
...
let command =
  Command.basic
    ...
    (iterations:int) () ->
      ...
      let tot_scale = match scale with   | Some x -> x *. syn_scale | None -> syn_scale in
