...
def setupMPPDefaults():
    ...
    pp.rcParams['axes.titlesize'] = 30
    pp.rcParams['axes.labelsize'] = 30
...
def plot_latency(rtt_file, paths_dir, out_file, rtt_hop):
    ...
    ax.set_xlabel("src-dst pairs sorted by shortest-path distance")
    if rtt_hop == 'rtt':
        ax.set_ylabel("Expected path RTT (ms)")
    else:
        ax.set_ylabel("Expected path hop count")
