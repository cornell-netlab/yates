...
let main algo topo_fn actual_fn predicted_fn hosts_fn init_str src_routing () =
  match algo with
  | Mcf -> let module C = Controller.Make(Mcf) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Vlb -> let module C = Controller.Make(Vlb) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Ecmp -> let module C = Controller.Make(Ecmp) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Ksp -> let module C = Controller.Make(Ksp) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Spf -> let module C = Controller.Make(Spf) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Ak -> let module C = Controller.Make(Ak) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Smcf -> let module C = Controller.Make(SemiMcf) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
  | Raeke -> let module C = Controller.Make(Raeke) in C.start topo_fn actual_fn hosts_fn init_str src_routing ()
