...
let data_plane_fault_constraints (pmap : path_uid_map)
  ...
  let failures_list = List.fold_left (range 0 !Yates_Globals.ffc_max_link_failures)
    ~init:[] ~f:(fun acc i ->
...
let ffc_mcf (topo:topology) (d:demands) (base_path_set : (path List.t) SrcDstMap.t) : scheme =
  ...
  let (unnormalized_scheme, flow_sum) = Yates_SemiMcf.scheme_and_flows flows umap in
  let new_scheme = Yates_SemiMcf.normalize unnormalized_scheme flow_sum in
