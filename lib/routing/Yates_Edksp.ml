...
let solve_lp (topo:topology) : scheme =
  ...
  if !Yates_Globals.er_mode then failwith "Not implemented" else
  ...
  let max_budget =
    if use_min_cut then
      min (min_st_cut topo src dst) (!Yates_Globals.budget)
    else
  ...
  let rec read_output gurobi solve_time =
    ...
    let tmp_scheme = Yates_Mcf.recover_paths topo flows_table in
    ...
    | None ->
      if use_min_cut then
        ...
        let ksp = k_shortest_paths topo src dst (min !Yates_Globals.budget 100) in
